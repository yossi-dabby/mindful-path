...
export default function MessageBubble({ message, conversationId, messageIndex, agentName = 'cbt_therapist', context = 'chat', userMessage, sessionLanguage }) {
  ...
  const attachmentFieldLocation = isUser && message.metadata?.attachment && typeof message.metadata.attachment === 'object' ?
  'metadata.attachment' :
        ...
        <div className="bg-teal-600 text-primary-foreground px-5 py-3 rounded-2xl shadow-[var(--shadow-sm)]">
          ...
          <>
                <div className="mb-2 rounded border border-slate-300 bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-800">
                  ATTACHMENT_FIELD_PATH_CHECK: metadata.attachment → attachment | DETECTED: {attachmentFieldLocation}
                </div>
                {attachment &&
