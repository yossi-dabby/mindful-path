...
const buildInitialStacks = () => Object.fromEntries(Object.keys(TAB_ROOTS).map((tab) => [tab, [{ path: `/${tab}`, pageName: tab }]]));
const initialState = {
    activeTab: 'Home',
...
function tabReducer(state, action) {
    switch (action.type) {
        ...
        case 'PUSH_PAGE': {
            ...
            if (stack[stack.length - 1]?.path === path)
                return state; // already top
            return {
                ...state,
...
export function TabNavigationProvider({ children, currentPageName }) {
    ...
    const canGoBack = useCallback(() => tabStacks[activeTab].length > 1, [activeTab, tabStacks]);
    ...
    return (_jsx(TabNavigationContext.Provider, { value: value, children: children }));
}
