...
export default function BottomNav({ currentPageName }) {
    ...
    return (_jsx("nav", { "aria-label": "Main navigation", className: "md:hidden fixed bottom-0 left-0 right-0 border-t border-border/70 bg-[hsl(var(--sidebar-background)/0.9)] backdrop-blur-2xl shadow-[var(--shadow-lg)]", style: {
            zIndex: 35,
            height: `calc(${BOTTOM_NAV_HEIGHT}px + env(safe-area-inset-bottom, 0px))`,
            paddingBottom: 'env(safe-area-inset-bottom, 0px)'
        }, children: _jsx("div", { className: "bg-emerald-300 px-1 flex justify-around items-center h-full", children: navItems.map((item) => {
                const Icon = item.icon;
                const isActive = currentPageName === item.path;
                return (_jsxs(Link, { to: createPageUrl(item.path), onClick: (e) => handleTabClick(e, item), "aria-current": isActive ? 'page' : undefined, "aria-label": item.name, className: "bg-teal-100 text-gray-950 px-2 py-2 rounded-3xl flex flex-col items-center justify-center gap-1 transition-calm min-w-[52px] border border-transparent", children: [_jsx(Icon, { className: "text-teal-700 lucide lucide-message-circle w-5 h-5 icon-default scale-110", strokeWidth: isActive ? 2.5 : 2 }), _jsx("span", { className: cn('text-xs font-medium leading-none', isActive ? 'text-primary' : 'text-muted-foreground'), children: item.name })] }, item.path));
            }) }) }));
}
