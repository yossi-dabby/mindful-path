...
export default function SocialAuthButtons() {
  ...
  return (
    <div className="space-y-3">
      <Button
        type="button"
        className="h-12 w-full rounded-2xl bg-teal-700 text-sm font-semibold text-white shadow-lg shadow-teal-700/20 hover:bg-teal-800"
        onClick={handle("google")}
      >
        <GoogleIcon className="me-2 h-5 w-5 rounded-full bg-white p-0.5" />
        {t("auth.social.continue_with", { provider: "Google" })}
      </Button>

      <button
        type="button"
        onClick={() => setShowAlternatives((value) => !value)}
        className="mx-auto flex min-h-11 items-center justify-center gap-1.5 rounded-xl px-3 text-xs font-semibold text-slate-500 hover:bg-slate-50 hover:text-teal-800"
        aria-expanded={showAlternatives}
      >
        {t("auth.social.other_options")}
        <ChevronDown className={`h-4 w-4 transition-transform ${showAlternatives ? "rotate-180" : ""}`} />
      </button>

      {showAlternatives && (
        <div className="space-y-2" data-testid="alternative-auth-providers">
          {ALTERNATIVE_PROVIDERS.map(({ id, label, Icon }) => (
            <Button
              key={id}
              type="button"
              variant="outline"
              className="h-11 w-full rounded-2xl border-slate-200 text-sm font-medium"
              onClick={handle(id)}
            >
              <Icon className="me-2 h-5 w-5" />
              {t("auth.social.continue_with", { provider: label })}
            </Button>
          ))}
        </div>
      )}
