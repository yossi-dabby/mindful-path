...
const { appId: _envAppId, token, functionsVersion } = appParams;

// App ID resolves from env var first, then app-params (URL/localStorage),
// then falls back to the known hardcoded app ID so it never resolves to
// null in Railway production when VITE_BASE44_APP_ID is absent at build time.
const APP_ID = import.meta.env.VITE_BASE44_APP_ID || _envAppId || '69504b725a07f5aa75aeaf7d';
