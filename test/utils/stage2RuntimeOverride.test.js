...
describe('Stage 2 Runtime Override — isolated V10 Knowledge Preview host', () => {
  const previewHostname = 'mindful-path-v10-preview.base44.app';

  const expectedEnabledFlags = new Set([
  ...
  it('enables exactly the four V10 Knowledge flags without a query override', () => {
    withWindowSearch('', () => {
      for (const flagName of Object.keys(THERAPIST_UPGRADE_FLAGS)) {
        expect(
          isUpgradeEnabled(flagName),
          `unexpected state for ${flagName}`,
        ).toBe(expectedEnabledFlags.has(flagName));
      }
    }, previewHostname);
  });

  it('does not allow _s2 to escalate the isolated host into later phases', () => {
    const search = [
      '?_s2=THERAPIST_UPGRADE_COMPETENCE_ENABLED',
      'THERAPIST_UPGRADE_PLANNER_FIRST_ENABLED',
      'THERAPIST_UPGRADE_MEMORY_ENABLED',
    ].join(',');

    withWindowSearch(search, () => {
      for (const flagName of expectedEnabledFlags) {
        expect(isUpgradeEnabled(flagName)).toBe(true);
      }

      expect(
        isUpgradeEnabled('THERAPIST_UPGRADE_COMPETENCE_ENABLED'),
      ).toBe(false);

      expect(
        isUpgradeEnabled('THERAPIST_UPGRADE_PLANNER_FIRST_ENABLED'),
      ).toBe(false);

      expect(
        isUpgradeEnabled('THERAPIST_UPGRADE_MEMORY_ENABLED'),
      ).toBe(false);
    }, previewHostname);
  });

  it('does not activate a deceptive sibling hostname', () => {
    withWindowSearch('?_s2=THERAPIST_UPGRADE_ENABLED', () => {
      expect(isUpgradeEnabled('THERAPIST_UPGRADE_ENABLED')).toBe(false);
      expect(
        isUpgradeEnabled('THERAPIST_UPGRADE_KNOWLEDGE_ENABLED'),
      ).toBe(false);
    }, `${previewHostname}.evil.example`);
  });
});
