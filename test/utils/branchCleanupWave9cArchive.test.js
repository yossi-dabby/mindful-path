...
} from '../../scripts/branch-cleanup-wave-9c-archive.mjs';

const REPO_ROOT = resolve(import.meta.dirname, '../..');
const WORKFLOW_PATH = resolve(REPO_ROOT, '.github/workflows/branch-cleanup-wave-9c-archive.yml');
...
const WAVE_9B_APPROVED_LIST_PATH = resolve(REPO_ROOT, 'docs/branch-cleanup-wave-9b-approved-list.txt');

describe('branch cleanup wave 9C reference scanning', () => {
  it('excludes Wave 9C approved list and audit source from blocking scans', () => {
  ...
  it('excludes Wave 9B approved list from blocking scans', () => {
    expect(
      REFERENCE_SCAN_EXCLUDED_PATHS.has('docs/branch-cleanup-wave-9b-approved-list.txt')
    ).toBe(true);
  });
  ...
  it('excluded paths do not block', () => {
    const refs = findReferences('copilot/example-branch', {
      ...
      runCommand: () =>
        ...
        'docs/branch-cleanup-wave-9b-approved-list.txt\n' +
  ...
  it('other docs/workflow references block', () => {
    const refs = findReferences('copilot/example-branch', {
      ...
      runCommand: () =>
        `${APPROVED_LIST_RELATIVE_PATH}\n` +
        'docs/branch-cleanup-wave-9c.md\n' +
        '.github/workflows/branch-cleanup-wave-9c-archive.yml\n',
    });
...
describe('branch cleanup wave 9C guardrails', () => {
  ...
  it('rejects branches already processed in Wave 9B', () => {
    const wave9bBranches = new Set(['copilot/stabilize-runtime-behavior']);
    ...
    ).toThrow(/already processed in Wave 9B/);
  });

  it('allows branches not in Wave 9A or Wave 9B list', () => {
    const wave9aBranches = new Set(['copilot/fix-lucide-inventory-workflow']);
    const wave9bBranches = new Set(['copilot/stabilize-runtime-behavior']);

    expect(() =>
      validateApprovedList({
        branches: ['copilot/audit-store-readiness'],
        wave9aBranches,
        wave9bBranches,
      })
    ).not.toThrow();
  ...
  it('archive tags follow required prefix and include date + branch', () => {
    ...
    expect(tag).toMatch(/^archive\/branch-cleanup-wave-9c\//);
  });
  ...
  it('parses required 30-90d audit section', () => {
    ...
    expect(auditMap.has('copilot/audit-store-readiness')).toBe(true);
    const record = auditMap.get('copilot/audit-store-readiness');
...
describe('branch cleanup wave 9C workflow', () => {
  ...
  it('exact Wave 9C confirmation string is required', () => {
    const workflow = readFileSync(WORKFLOW_PATH, 'utf8');

    expect(workflow).toContain(
      'CONFIRM_ARCHIVE_AND_DELETE_CLOSED_UNMERGED_30_90D_WAVE_9C'
    );
    ...
    expect(workflow).not.toContain('CONFIRM_ARCHIVE_AND_DELETE_CLOSED_UNMERGED_30_90D_WAVE_9B');
  });
  ...
  it('workflow uploads report artifact', () => {
    ...
    expect(workflow).toContain('branch-cleanup-wave-9c-report');
    expect(workflow).toContain('upload-artifact');
  });
});

describe('branch cleanup wave 9C approved list', () => {
  it('approved list has exactly 16 branches', () => {
    const branches = parseApprovedBranches(readFileSync(APPROVED_LIST_PATH, 'utf8'));
    expect(branches.length).toBe(16);
  });

  it('approved list does not exceed the 25 branch cap', () => {
    const branches = parseApprovedBranches(readFileSync(APPROVED_LIST_PATH, 'utf8'));
  ...
  it('approved list does not overlap with Wave 9A approved list', () => {
    const wave9cBranches = parseApprovedBranches(readFileSync(APPROVED_LIST_PATH, 'utf8'));
    const wave9aBranches = new Set(
      parseApprovedBranches(readFileSync(WAVE_9A_APPROVED_LIST_PATH, 'utf8'))
    );

    for (const branch of wave9cBranches) {
      expect(
        wave9aBranches.has(branch),
        `Branch "${branch}" appears in both Wave 9A and Wave 9C approved lists`
      ).toBe(false);
    }
  });

  it('approved list does not overlap with Wave 9B approved list', () => {
    const wave9cBranches = parseApprovedBranches(readFileSync(APPROVED_LIST_PATH, 'utf8'));
    ...
    for (const branch of wave9cBranches) {
      expect(
        wave9bBranches.has(branch),
        `Branch "${branch}" appears in both Wave 9B and Wave 9C approved lists`
      ).toBe(false);
    }
  });
