...
const APPROVED_FORMS = ALL_FORMS.filter((f) => f.approved === true && f.type !== 'therapeutic_workbook' && f.category !== 'children_cbt_process' && f.languages?.pt);
...
describe('Portuguese integration — AI mappings unchanged', () => {
  ...
  it('34. Every AI mapping intent resolves to an approved standard form', () => {
  ...
  it('35. Every AI mapping intent still resolves in Portuguese after the update (standard forms)', () => {
    for (const [intent, formId] of Object.entries(APPROVED_FORM_INTENT_MAP)) {
      if (WORKBOOK_FORM_IDS.has(formId)) continue; // Hebrew-only workbooks only resolve in Hebrew
