...
describe('Italian integration — AI mappings unchanged', () => {
  ...
  it('33. Every AI mapping intent resolves to an approved standard form', () => {
  ...
  it('34. Every AI mapping intent still resolves in Italian after the update (standard forms)', () => {
    for (const [intent, formId] of Object.entries(APPROVED_FORM_INTENT_MAP)) {
      if (WORKBOOK_FORM_IDS.has(formId)) continue; // Hebrew-only workbooks only resolve in Hebrew
