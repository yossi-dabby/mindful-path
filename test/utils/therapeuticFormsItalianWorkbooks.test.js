...
describe('Italian workbooks — regressions', () => {
  ...
  it('existing 18 individual forms still resolve', () => {
    const approvedIndividuals = ALL_FORMS.filter(f => f.approved === true && f.category !== 'workbook_series' && f.category !== 'children_cbt_process' && f.languages?.it);
  ...
  it('total approved counts reflect +7 Portuguese workbooks on top of prior languages', () => {
    ...
    expect(approvedWorkbooks.length).toBe(77);
