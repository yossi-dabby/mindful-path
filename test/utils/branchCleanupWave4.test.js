...
} from '../../scripts/branch-cleanup-wave-4.mjs';

const REPO_ROOT = resolve(import.meta.dirname, '../..');
const WORKFLOW_PATH = resolve(REPO_ROOT, '.github/workflows/branch-cleanup-wave-4.yml');
const APPROVED_LIST_PATH = resolve(REPO_ROOT, 'docs/branch-cleanup-wave-4-approved-list.txt');
...
describe('branch cleanup wave 4 reference scanning', () => {
  it('ignores branch references found only in the wave-4 approved-list manifest', () => {
  ...
  it('keeps references found in other docs files', () => {
    const refs = findReferences('copilot/example-branch', {
      ...
      runCommand: () =>
        `${APPROVED_LIST_RELATIVE_PATH}\n` +
        'docs/branch-cleanup-wave-4.md\n' +
        'docs/some-other-doc.md\n',
    });

    expect(refs).toEqual(['docs/branch-cleanup-wave-4.md', 'docs/some-other-doc.md']);
  });
  ...
  it('keeps references found in workflow files', () => {
    const refs = findReferences('copilot/example-branch', {
      ...
      repoRoot: '/virtual/repo',
      pathExists: () => true,
      runCommand: () => '.github/workflows/branch-cleanup-wave-4.yml\n',
    });

    expect(refs).toEqual(['.github/workflows/branch-cleanup-wave-4.yml']);
  });
...
describe('branch cleanup wave 4 guardrails', () => {
...
describe('branch cleanup wave 4 workflow', () => {
  ...
  it('requires the exact Wave 4 confirmation string', () => {
    const workflow = readFileSync(WORKFLOW_PATH, 'utf8');

    expect(workflow).toContain('CONFIRM_DELETE_SAFE_MERGED_BRANCHES_WAVE_4');
...
describe('branch cleanup wave 4 approved list', () => {
