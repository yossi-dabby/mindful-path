...
function stageOf(result) {
  ...
  const match = id?.match(/adolescents-cbt-core-(\d)-\d-en$/);
...
describe('Adolescent CBT Core EN — registry and assets', () => {
  it('registers exactly 30 individual forms from the manifest', () => {
    expect(FORMS_ADOLESCENTS_CBT_CORE_EN_INDIVIDUAL).toHaveLength(30);
    expect(FORMS_ADOLESCENTS_CBT_CORE_EN).toHaveLength(30);
  });

  it('all forms are approved and include required content-aware metadata fields', () => {
    for (const form of FORMS_ADOLESCENTS_CBT_CORE_EN_INDIVIDUAL) {
      ...
      expect(form.series).toBe('adolescents-cbt-core');
  ...
  it('manifest/readme/qa files and all registered file URLs exist on disk', () => {
    ...
    for (const form of FORMS_ADOLESCENTS_CBT_CORE_EN) {
  ...
  it('manifest remains the canonical source for all worksheet entries', () => {
    ...
    const manifestIds = ADOLESCENTS_CBT_CORE_EN_MANIFEST.forms.map((entry) => entry.id);
    const registryIds = FORMS_ADOLESCENTS_CBT_CORE_EN_INDIVIDUAL.map((form) => form.id);
    expect(new Set(manifestIds)).toEqual(new Set(registryIds));
  });
});
...
describe('Adolescent CBT Core EN — forms library visibility', () => {
  ...
  it('therapeutic forms page merges adolescent CBT core English forms into the library registry', () => {
    expect(pageSource).toContain('FORMS_ADOLESCENTS_CBT_CORE_EN_INDIVIDUAL');
    expect(pageSource).toContain('...FORMS_ADOLESCENTS_CBT_CORE_EN_INDIVIDUAL');
  });
});

describe('Adolescent CBT Core EN — content-aware resolver prompts A–H', () => {
  it('resolves direct form IDs via resolveFormIntent', () => {
    const result = resolveFormIntent('adolescents-cbt-core-1-1-en', 'en');
    expect(idOf(result)).toBe('adolescents-cbt-core-1-1-en');
  });

  it('A: overwhelmed teen query maps to stage 1', () => {
    ...
    expect(['adolescents-cbt-core-1-1-en', 'adolescents-cbt-core-1-2-en', 'adolescents-cbt-core-1-4-en']).toContain(idOf(result));
  });

  it("B: automatic thoughts query maps to stage 2", () => {
    const result = resolveAdolescentsCBTCoreEnglishFormByContent("Teen has automatic thoughts like I won't succeed.");
    expect(stageOf(result)).toBe(2);
  });

  it('C: evidence-checking query maps to stage 3', () => {
  ...
  it('D: choosing action query maps to stage 4', () => {
    const result = resolveAdolescentsCBTCoreEnglishFormByContent('Teen needs help choosing what action to take.');
    expect(stageOf(result)).toBe(4);
  });

  it('E: avoidance/exposure query maps to stage 5', () => {
    const result = resolveAdolescentsCBTCoreEnglishFormByContent('Teen avoids things and needs gradual exposure.');
    expect(stageOf(result)).toBe(5);
  });

  it('F: weekly check-in/personal plan query maps to stage 6', () => {
    const result = resolveAdolescentsCBTCoreEnglishFormByContent('Teen needs a weekly check-in and encouragement plan.');
    expect(stageOf(result)).toBe(6);
  });

  it('H: child query does not force adolescent core forms', () => {
    const result = resolveAdolescentsCBTCoreEnglishFormByContent('Child age 8 has fears.');
...
describe('Adolescent CBT Core EN — canonical manifest catalog', () => {
  ...
  it('includes canonical English adolescent CBT core section with metadata guidance', () => {
    expect(catalog).toContain(heading);
    expect(catalog).toContain('Do NOT mix in Hebrew adolescent core/specialized forms, children forms, adult forms, or stale titles.');
    expect(catalog).toContain('therapeuticGoal');
    expect(catalog).toContain('shortContentDescription');
    expect(catalog).toContain('intentPhrases');
  });

  it('G: canonical section contains exactly the 30 manifest worksheet markers and no unrelated mix-ins', () => {
    const section = catalog.slice(catalog.indexOf(heading));
    const ids = section.match(/\[FORM:adolescents-cbt-core-[1-6]-[1-5]-en\]/g) || [];
    expect(ids).toHaveLength(30);
    expect(section).not.toContain('tf-children-cbt');
    expect(section).not.toContain('tf-adolescents-cbt-specialized');

    for (const entry of ADOLESCENTS_CBT_CORE_EN_MANIFEST.forms) {
      expect(section).toContain(`[FORM:${entry.id}]`);
