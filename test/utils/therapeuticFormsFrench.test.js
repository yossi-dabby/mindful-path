...
const APPROVED_FORMS = ALL_FORMS.filter((f) => f.approved === true && f.type !== 'therapeutic_workbook' && f.category !== 'children_cbt_process' && f.languages?.fr);
...
describe('French integration — AI mappings unchanged', () => {
  ...
  it('28. Every AI mapping intent resolves to an approved standard form', () => {
  ...
  it('29. Every AI mapping intent still resolves in French after the update (standard forms)', () => {
    for (const [intent, formId] of Object.entries(APPROVED_FORM_INTENT_MAP)) {
      if (WORKBOOK_FORM_IDS.has(formId)) continue; // Hebrew-only workbooks only resolve in Hebrew
