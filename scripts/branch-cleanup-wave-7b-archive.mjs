 ...
 *   - Approved list must be non-empty and must not exceed 50 entries
 ...
 *     (excluding docs/branch-cleanup-wave-7b-approved-list.txt and docs/remaining-branch-reduction-audit.md)
...
const APPROVED_LIST_RELATIVE_PATH = 'docs/branch-cleanup-wave-7b-approved-list.txt';
...
const REPORT_PATH = resolve(REPO_ROOT, 'branch-cleanup-wave-7b-report.md');
...
function validateApprovedBranches(branches, currentBranch = '') {
  if (branches.length === 0) {
    throw new Error('Approved list is empty. Add at least one Wave 7B branch before running cleanup.');
  }
...
function createAnnotatedTag(tagName, targetSha, branch) {
  const message = [
    'Branch Cleanup Wave 7B archive tag',
    `Branch: ${branch}`,
...
async function writeReport({
  ...
  const lines = [
    '# Branch Cleanup Wave 7B Archive — Report',
    '',
    ...
    '',
    '- ✅ Approved list was non-empty and did not exceed 50 entries',
