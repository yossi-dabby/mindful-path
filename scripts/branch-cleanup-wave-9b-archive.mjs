/**
 * Branch Cleanup Wave 9B (Archive Tag + Delete)
 *
 * Safely archives and deletes only explicitly approved closed-unmerged Copilot branches
 * aged 30–90 days (medium-risk bucket, batch 2 of up to Wave 9 splits).
 *
 * Safety rules (all must pass before any branch deletion):
 *   - Approved list must be non-empty and must not exceed 40 entries
 ...
 *   - Branch must not have been processed in Wave 9A
 *   - Branch must have no blocking references in workflows/docs/README/package/deploy config
 *     (excluding docs/branch-cleanup-wave-9b-approved-list.txt and the audit source)
...
export const APPROVED_LIST_RELATIVE_PATH = 'docs/branch-cleanup-wave-9b-approved-list.txt';
...
const REPORT_PATH = resolve(REPO_ROOT, 'branch-cleanup-wave-9b-report.md');

export const MAX_BRANCHES = 40;
export const ARCHIVE_TAG_PREFIX = 'archive/branch-cleanup-wave-9b';
...
export function validateApprovedList({ branches, currentBranch = '', wave9aBranches = new Set() }) {
  if (branches.length === 0) {
    throw new Error(
      'Approved list is empty. Add at least one Wave 9B branch before running cleanup.'
    );
  }
...
function createAnnotatedTag(tag, sha, branch) {
  run(`git tag -a "${tag}" "${sha}" -m "Archive: ${branch} before Wave 9B branch deletion"`, {
    throws: true,
  });
}
    ...
    const req = https.request(
      url,
      {
        ...
        headers: {
          ...
          'User-Agent': 'branch-cleanup-wave-9b',
...
async function writeReport({
  ...
  const lines = [
    ...
    '',
    '- ✅ Approved list was loaded and validated with strict cap of 40',
    '- ✅ Duplicate branch names were rejected',
    '- ✅ Protected/special/current/dangerous branch names were blocked',
    '- ✅ Wave 9A branches were verified absent from Wave 9B list',
...
async function main() {
  ...
  validateApprovedList({ branches, currentBranch, wave9aBranches });
  ...
  const context = { owner, repo, currentBranch, auditMap, wave9aBranches };
