 ...
 *     deployment configs
 *   - Approved list must not exceed 75 entries
 *
 * Usage (invoked by the GitHub Actions workflow only):
 *   node scripts/branch-cleanup-wave-5.mjs
 *
 * Required env vars:
...
const APPROVED_LIST_RELATIVE_PATH = 'docs/branch-cleanup-wave-5-approved-list.txt';
const APPROVED_LIST_PATH = resolve(REPO_ROOT, 'docs/branch-cleanup-wave-5-approved-list.txt');
const REPORT_PATH = resolve(REPO_ROOT, 'branch-cleanup-wave-5-report.md');
...
async function openPrCount(owner, repo, branch) {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'branch-cleanup-wave-5',
    },
  });
...
async function writeReport({ startTime, owner, repo, branches, results, aborted, remoteInventory }) {
  ...
  const lines = [
    ...
    '- ✅ Approved list did not exceed 75 entries',
