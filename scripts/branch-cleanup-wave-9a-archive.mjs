/**
 * Branch Cleanup Wave 9A (Archive Tag + Delete)
 *
 * Safely archives and deletes only explicitly approved closed-unmerged Copilot branches
 * aged 30–90 days (medium-risk bucket, batch 1 of up to Wave 9 splits).
 *
 ...
 *     (excluding docs/branch-cleanup-wave-9a-approved-list.txt and the audit source)
...
export const APPROVED_LIST_RELATIVE_PATH = 'docs/branch-cleanup-wave-9a-approved-list.txt';
...
const REPORT_PATH = resolve(REPO_ROOT, 'branch-cleanup-wave-9a-report.md');

export const MAX_BRANCHES = 25;
export const ARCHIVE_TAG_PREFIX = 'archive/branch-cleanup-wave-9a';
...
export function validateApprovedList({ branches, currentBranch = '' }) {
  if (branches.length === 0) {
    throw new Error(
      'Approved list is empty. Add at least one Wave 9A branch before running cleanup.'
    );
  }
...
function createAnnotatedTag(tag, sha, branch) {
  run(`git tag -a "${tag}" "${sha}" -m "Archive: ${branch} before Wave 9A branch deletion"`, {
    throws: true,
  });
}
    ...
    const req = https.request(
      url,
      {
        ...
        headers: {
          ...
          'User-Agent': 'branch-cleanup-wave-9a',
        ...
        res.on('end', () => {
          if (res.statusCode === 200) {
            try {
              ...
              rejectPromise(
                new Error(`Failed to parse GitHub API response for branch "${branch}".`)
              );
            }
            return;
          }
...
async function evaluateBranch(
  branch,
  { owner, repo, currentBranch, auditMap },
  {
...
async function main() {
  ...
  validateApprovedList({ branches, currentBranch });
  ...
  const context = { owner, repo, currentBranch, auditMap };
