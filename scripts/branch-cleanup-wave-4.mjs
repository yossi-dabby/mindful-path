 ...
 *   node scripts/branch-cleanup-wave-4.mjs
 *
 * Required env vars:
...
const APPROVED_LIST_RELATIVE_PATH = 'docs/branch-cleanup-wave-4-approved-list.txt';
const APPROVED_LIST_PATH = resolve(REPO_ROOT, 'docs/branch-cleanup-wave-4-approved-list.txt');
const REPORT_PATH = resolve(REPO_ROOT, 'branch-cleanup-wave-4-report.md');
...
async function openPrCount(owner, repo, branch) {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'branch-cleanup-wave-4',
    },
  });
